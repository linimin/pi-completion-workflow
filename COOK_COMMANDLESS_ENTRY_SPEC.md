...
- Builds on: `COOK_NATURAL_LANGUAGE_TRIGGER_PLAN.md` 已完成的 router-mode 自然語言 routing 基礎
...
- 在主 session 討論後，用自然語言 handoff（例如 `開始做` / `開始實作` / `go ahead`）先攔截，再由 extension 提供 router confirmation / clarification
...
這和現在 router confirm-first 設計一致：workflow offer 是獨立控制面，不是把原 prompt 悄悄改路由。
...
```json
{
  ...
  "scope": ["input interception", "classifier", "router confirmation", "tests"],
...
### Phase 1：commandless startup router
